...
describe('Merge two sorted arrays with smaller auxiliary array', () => {
  it('test', () => {
    ...
    const comparator = (n1: number, n2: number): -1 | 0 | 1 => {
      if (n1 === n2) return 0;
      return n1 < n2 ? -1 : 1;
    };
    ...
    expect(isCorrectOrder(arr, comparator));
  });
...
function merge<T>(arr: T[], lo: number, mid: number, hi: number, comparator: (t1: T, t2: T) => -1 | 0 | 1): void {
  ...
  for (let k = lo; k <= hi; k++) {
    ...
    else if (comparator(aux[i], arr[j]) <= 0) arr[k] = aux[i++];
    else arr[k] = arr[j++];
  }
  return;
}

function sort<T>(arr: T[], lo: number, hi: number, comparator: (t1: T, t2: T) => -1 | 0 | 1): void {
  if (lo >= hi) return;
